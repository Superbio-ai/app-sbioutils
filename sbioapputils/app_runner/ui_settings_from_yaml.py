...
def _define_files_from_yaml(yaml_dict):
    input_files = []
    for file_key, file_data in yaml_dict['input_settings']['upload_options'].items():
        template_type = file_data['type']
        if template_type == 'table':
            input_element = csv_template.copy()
        elif template_type == 'image':
            input_element = image_template.copy()
        elif template_type == 'single_cell':
        ...
        input_element['name'] = file_key
        input_element['title'] = file_data['title']
        if file_data.get('demo_path'):
            input_element['demoDataDetails'] = {
                'description': file_data['demo_description'],
                'filePath': file_data['demo_path'],
                'fileName': file_data['demo_path'].split('/')[-1],
                'fileSource': [{'title': 'Data Source', 'url': file_data['url']}]
            }
...
def define_settings_from_yaml(yaml_loc):
        ...
        if value['type'] in ['int', 'float']:
            ...
            if value.get('from_data') == 'True':
                ...
                input_parameters.append({
                    ...
                    "tooltip": value['tooltip'],
                    "type": 'str',
                    "default_value": {'label': value['default'], 'value': value['default']},
                    "input_type": "dropdown",
                    "options": []
                })
    ...
    if yaml_dict.get('output_settings', {}).get('description'):
        results_config['description'] = yaml_dict['output_settings']['description']
    if yaml_dict.get('output_settings', {}).get('save_model'):
